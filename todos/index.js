...
// TODO: Implement file reading and writing function
const file = {
  read: () => {
    console.log('read');
  },
...
const todos = {
  get: (req, res) => {
    TODOS.todos
      ...
      : res
          .status(400)
          .send(
            errorMessage('Your todo list is empty. Please add new todo list')
          );
  },
  ...
  add: (req, res) => {
    ...
    if (todoTask && todoPriority && todoDeadline) {
      ...
      res
        .status(400)
        .send(errorMessage('You have to fill all of your data properties'));
    }
  },

  getOnebyCharacters: (req, res) => {
    const searchedTask = String(req.query.keyword).toLowerCase();
    ...
    if (searchedTask) {
      ...
      if (todoData) res.status(200).send(todoData);
        ...
        res.status(400).send(errorMessage('Task you have searched is empty'));
    } else {
    ...
    if (updatedId) {
      if (todoTask && todoPriority && todoDeadline) {
        ...
        if (todoData) {
          ...
          todoData.deadline = new Date(todoDeadline);
          ...
          res
            .status(400)
            .send(errorMessage(`Data with ID ${updatedId} doesn't exists`));
        }
      } else {
        res
          .status(400)
  ...
  deleteTodobyId: (req, res) => {
    ...
    if (searchedId) {
      ...
      if (todoData) {
        ...
        // TODO: UPDATE DATA IN JSON AS WELL

        res
          .status(200)
          .send({ message: `Data with ID ${searchedId} has been deleted` });
      } else {
      ...
      res
        .status(400)
        .send(errorMessage('Please choose task you want to delete'));
    }
  }
};
